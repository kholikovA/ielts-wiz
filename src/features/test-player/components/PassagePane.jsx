...
function HeadingGap({ qn, mhKey, place, answers }) {
  const val = answers[qn];
  return (
    <div
      className="heading-gap"
      data-qnum={qn}
      onClick={() => (val ? place.clearAt(qn) : place.placeAt(mhKey, qn))}
      onDragOver={(e) => e.preventDefault()}
      onDrop={(e) => place.dropAt(mhKey, qn, e)}
    >
...
export default function PassagePane({ spec, mhByPart, place, answers }) {
  return (
    <>
                ...
                if (isMH) {
                  ...
                  return (
                    <React.Fragment key={idx}>
                      {qn && <HeadingGap qn={qn} mhKey={mh.key} place={place} answers={answers} />}
                      <p><HTML html={text} /></p>
                    </React.Fragment>
                  );
                }
